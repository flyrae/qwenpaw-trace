...
| （v0.4.4） | **去估算化**：输入构成桶改为字符数+占比（真实字符），新增「输入 token（实测）」行（provider usage 计费口径，含带外工具结果）；删除每桶 ~tok 估算与模型系数标注；note 说明两口径差异 |
| （v0.4.3） | **尾部更新识别（修重置风暴）**：真实会话发现运行时在 ReAct 步间**原地改写末条 assistant 消息**（内容逐轮增长），旧规则只认前缀追加 → 每次调用误判整上下文重置并全量重记（9 条 × N 次重复 + 移除/新增误导展示）。修复：`llm_input_delta` 三态返回（append / **tail_update**（LCP=旧长−1 且同 role → 只记替换条）/ reset），`llm/call.tail_update` 落字段；前端重构 accumulatedInput 按 tail_update **替换末条**；无 reset 标志的全量 messages_new 视为指纹缓存丢失（热装）→ resync 替换而非追加；重置详情 removed/added 加「旧:/新:」前缀消歧；99 测试（+尾部增长用例） |
| （v0.4.2） | **UX 打磨（P1+P2）**：工具栏「图例」弹层（📚/⚡/∈/色条/色带五符号说明）；输入页消息列表收进可折叠面板（>5 条默认收起）+ 每条 >200 字截 3 行带「展开全文」；请求 pill 📚 可点直达 SpanInspector（wrapper span 兜底 antd 版本差异）；色带 6→10px、antd Tooltip、hover 高亮、宽度足够时内嵌技能名、pointerdown 阻断与轨迹拖选竞态；重置状态词双语（保留/重写/移除/新增）；单技能会话隐藏行色条（bypass 橙条保留）；∈ 归属降为静默彩色文本（事实用标签、推断不用）；实测 pill/折叠/图例/色带渲染通过 |
| （v0.4.1） | **重置详情定位**：`context_reset` 从布尔升级为可定位——折叠层用 messages_new 增量重构旧输入、与重置当次全量对比（`diffContextReset` 纯函数：LCP 断点 + 前后条数/字符 + role 变化 + 逐条 kept/removed/rewritten/added，同 role 残余配对为 rewritten）；旧轨迹回退 digest 结构级对比；「输入」页警告下展开详情卡（断点 #N、前后规模、role 变化、逐条状态前 20 条）；node 测试补 8 例；合成轨迹实测数值全对 |
| （v0.4.0） | **采集期技能字段（WP5）**：`run/start.slash_skill` + `trigger=skill_command`（斜杠分发在捕获期落字段，读端不再正则）；`tool/call.skill_resource`（中间件解析 `<agent-skills>` 目录映射——service 会话级缓存，命令含目录路径即打标，第 3 层从查看器启发式升级为采集事实）；`tool/result.skill_sha`（SKILL.md 内容哈希，前端索引未变时跳过重建）；store 两处折叠新增 `skills_detail {loads/slash_runs/resource_calls}`（`skills` 兼容保留）；98 测试；**写路径变更，需重启生效** |
| （v0.3.9） | **内容匹配证据（WP4）**：加载技能时对 SKILL.md 正文建特征索引（脚本文件名 + 围栏命令特异 token，通用词表过滤）；后续工具调用命中特征 → `content` 证据（介于 path 事实与 temporal 推断之间），悬停可见命中特征串；多技能歧义不硬选（回退 temporal）；索引每次加载重建（技能编辑后自动失效）；node 测试补 7 例 |
